...
function renderYourView(items) {
    ...
    const photoTiles = items.length
        ? items.map(p => `<div class="bento-item customer-photo-item"><img src="${p.src}" alt="${p.alt}"><div class="customer-photo-overlay"></div></div>`).join('')
        : `<div class="bento-item customer-photo-item is-placeholder"><span>No photos shared yet</span></div>`;
    grid.innerHTML = photoTiles + `
...
function renderMemoryDetail(memoryId) {
    ...
    document.querySelector('.memory-detail-quote').textContent = `"${m.description}"`;
    document.querySelector('.tag-tertiary').textContent = m.date;
    document.querySelector('.tag-secondary').textContent = m.category;
    document.querySelector('.memory-detail-order-title').textContent = m.orderTitle;
    document.querySelector('.memory-detail-order-desc').textContent = m.orderDesc;
    document.getElementById('memory-detail-your-view-count').textContent = m.yourViewCount;
...
function initWelcomeScreen() {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        document.querySelectorAll('.welcome-floating-element, .welcome-floating-reverse').forEach((el, index) => {
...
function initEditProfileScreen() {
    ...
    if (editProfileScreen) {
        ...
        inputs.forEach(input => {
            input.addEventListener('focus', () => {
                const label = input.closest('.edit-profile-input-group').querySelector('.edit-profile-label');
                if (label) label.style.color = 'var(--color-primary)';
            });
            input.addEventListener('blur', () => {
                const label = input.closest('.edit-profile-input-group').querySelector('.edit-profile-label');
        ...
        saveBtn.addEventListener('click', () => {
            ...
            document.querySelector('.profile-user-name').textContent = nameInput.value;
            document.querySelector('.profile-user-email').textContent = emailInput.value;
...
function initEditMemoryScreen() {
    ...
    const checkChanges = () => {
        ...
        if (!m) return;
        saveBtn.disabled = (titleInput.value === m.title && descInput.value === m.description);
    };
...
function initShareMemoryScreen() {
    ...
    const openSheet = () => {
        ...
        if (m) {
            document.querySelector('.share-link-url').textContent = `memento.app/s/${m.linkSlug}`;
